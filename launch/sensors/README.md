...
| start_imu.launch | Starts the namespaced Xsens driver on the stable IMU device path and confines every data, time, magnetic, vector, and quaternion output below `/sensors/imu`. | xsens_mti_driver, config/udev/99-ig-handle.rules | scripts/sensors/external_sensor_provider.py through the sensor contract |

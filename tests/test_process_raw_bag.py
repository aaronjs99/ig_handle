...
class TestIgPostProcess(unittest.TestCase):
    def setUp(self):
        ...
        self.modules_patcher = patch.dict(sys.modules, {
            "rospy": self.mock_rospy,
            "rosbag": self.mock_rosbag
        })
    ...
    def test_restamp_logic(self):
        ...
        def read_side_effect(topics=None):
            ...
            t1 = MagicMock(); t1.time_ref = FakeTime(1011)
            t2 = MagicMock(); t2.time_ref = FakeTime(1021)
            time_msgs = [
                ("/time", t1, 1011),
                ("/time", t2, 1021)
            ]
        ...
        data_writes = [
            c for c in out_bag.write.call_args_list 
            if c[0][0] == "/data"
        ]
